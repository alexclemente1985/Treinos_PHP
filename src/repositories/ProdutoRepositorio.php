<?php
   class ProdutoRepositorio{
    ...
    public function opcoesProduto(string $nomeProduto): array{
        $sql = "SELECT * FROM produtos WHERE tipo = '$nomeProduto'";
        $statement = $this-> pdo -> query($sql);
        $produtos = $statement->fetchAll(PDO::FETCH_ASSOC);

        $dados = array_map(function ($p){
            return new Produto($p['id'],
                $p['tipo'],
                $p['nome'],
                $p['descricao'],
                floatval($p['preco']),
    ...
    public function deletar(int $id){
        $sql = "DELETE FROM produtos WHERE id = ?"; //proteção contra sql injection
